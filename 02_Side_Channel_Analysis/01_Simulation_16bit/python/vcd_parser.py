...
def extract_hd_from_vcd(vcd_path: str, target_signals: List[str]) -> Generator[Dict[str, int], None, None]:
    """
    Stream-parses a VCD file and yields the Hamming Distance of target signals
    at each timestamp where they change. 
    """
    signal_ids = {}
    current_values = {}
    previous_values = {}
...
if __name__ == "__main__":
    vcd_file = "data/traces/sim_trace.vcd"
    ...
    try:
        ...
        with open("data/traces/extracted_hd.json", "w") as f:
            json.dump(traces, f)
